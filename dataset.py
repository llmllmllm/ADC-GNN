...
class Dataset:
    def __init__(self, load_epoch, name='amazon', del_ratio=0., homo=True, data_path='', adj_type='sym'):
        self.name = name
        graph = None
        prefix = data_path
        if name == 'tfinance':
            ...
            if del_ratio != 0.:
                ...
                with open(f'probs_tfinance_BWGNN_{load_epoch}_{homo}.pkl', 'rb') as f:
                    ...
                    if torch.is_tensor(pred_y):
                        ...
                        pred_y = torch.tensor(pred_y).cpu()
            ...
            dataset = FraudDataset(name, train_size=0.01, val_size=0.33)
            graph = dataset[0]
            if homo:
                ...
                if del_ratio != 0.:
                    ...
                    for relation in graph.canonical_etypes:
                        graph_r = dgl.to_homogeneous(graph[relation],
                                                     ndata=['feature', 'label', 'train_mask', 'val_mask', 'test_mask'])
