...
def info_nce_loss(feat1, feat2, temperature=0.1):
...
class GaussianDiffusion(nn.Module):
    ...
    def p_loss_contrastive(
        ...
        x_noisy1 = self.q_sample(x_0, t, noise1)
        x_noisy2 = self.q_sample(x_0, t, noise2)
        
        noise_pred1, feat1 = model(x_noisy1, t)
        noise_pred2, feat2 = model(x_noisy2, t)

        mse_loss = F.mse_loss(noise_pred1, noise1)
        contrast_loss = info_nce_loss(feat1, feat2, temperature)
    ...
    def p_mean_variance(self, model, x, t, clip_denoised):
        x_recon = self.predict_start_from_noise(x, t, noise=model(x, t)[0])
        if clip_denoised:
